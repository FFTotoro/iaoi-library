...
namespace IAOI_lib{
  ...
  namespace internal{
    ...
    poly and_fwt(int n,poly a,int x){
    ...
    poly or_fwt(int n,poly a,int x){
      auto b=a; self_add(x,p);
      for(int i=0;i<n;i++)
        for(int j=0;j<a.size();j+=1<<i+1)
          for(int k=0;k<1<<i;k++)
    ...
    poly xor_fwt(int n,poly a,int x){
      auto b=a; self_add(x,p);
      for(int i=0;i<n;i++)
        for(int j=0;j<a.size();j+=1<<i+1)
          for(int k=0;k<1<<i;k++){
